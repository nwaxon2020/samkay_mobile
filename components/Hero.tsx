...
export default function Hero() {
    ...
    return (
        <section className="pt-6 relative overflow-hidden bg-white">
            ...
            {isOverlayOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in">
                    <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl relative border border-gray-100 scale-in-center">
                        <button onClick={() => setIsOverlayOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-red-500 transition-colors">
                            <FiXCircle size={28} />
                        </button>
                        <div className="text-center">
                            ...
                            <button onClick={() => setIsOverlayOpen(false)} className="w-full py-3 bg-gray-900 text-white font-bold rounded-xl hover:bg-gray-800 transition-all shadow-lg active:scale-95">I Understand</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="container mx-auto px-4 md:px-8 pb-8 md:py-16 relative z-10">
                    ...
                    <div className="lg:hidden">
                            ...
                            <div className="relative mx-auto w-full max-w-[18rem] md:max-w-sm">
                                <div className={`relative z-10 ${heroData.settings.animations ? 'animate-phone-float' : ''}`}>
                                    <div className="relative w-full h-[460px] mx-auto bg-gradient-to-b from-gray-900 to-black rounded-[2.5rem] shadow-2xl border-[12px] sm:border-[14px] border-gray-800 overflow-hidden">
                                        ...
                                        {heroData.featuredProduct.heroImage && (
                                            <img src={heroData.featuredProduct.heroImage} alt={heroData.featuredProduct.name} className="absolute inset-0 w-full h-full object-cover" />
                                        )}

                                        {/* SHARED DATA OVERLAY (Floats over image or black bg) */}
                                        <div className="absolute inset-2 sm:inset-3 bg-gradient-to-b from-black/20 via-transparent to-black/60 rounded-[1.5rem] sm:rounded-[1.8rem] overflow-hidden">
                                            ...
                                            <div className="relative p-4 sm:p-6 h-full flex flex-col justify-between">
                                                <div>
                                                    ...
                                                    <div className="text-white text-xl sm:text-2xl font-bold mb-1 drop-shadow-md">{heroData.featuredProduct.name}</div>
                                                    <div className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-[#F8AE1B] to-yellow-400 bg-clip-text text-transparent drop-shadow-md">{heroData.featuredProduct.model}</div>
                                                    <div className="bg-[rgba(0,0,0,0.7)] p-1 rounded-lg text-gray-200 text-xs sm:text-sm mt-1.5 flex items-center gap-1.5 drop-shadow-md"><FiZap className="text-[#F8AE1B]" size={12} />{heroData.featuredProduct.tagline}</div>
                                                </div>
                                        ...
                                        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-28 h-5 sm:w-32 sm:h-6 bg-black rounded-b-xl border-x border-b border-gray-700 z-30"></div>
                                    </div>
                                    
                                    {heroData.settings.floatingElements && heroData.settings.animations && (
                                        <>
                                            <div className="absolute -top-4 -left-6 w-16 h-16 bg-gradient-to-br from-gray-800 to-black rounded-xl border border-gray-700/50 flex items-center justify-center shadow-lg animate-float z-40" style={{ animationDelay: '0.5s' }}>
                                                <div className="text-xl text-white">{heroData.accessories[0].icon}</div>
                                                <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-[#F8AE1B] rounded-full flex items-center justify-center text-[8px] text-black font-bold">{heroData.accessories[0].tag}</div>
                                            </div>
                                            <div className="absolute -bottom-4 -right-6 w-20 h-20 bg-gradient-to-br from-gray-800 to-black rounded-xl border border-gray-700/50 flex items-center justify-center shadow-lg animate-float z-40" style={{ animationDelay: '1s' }}>
                                                <div className="text-2xl text-white">{heroData.accessories[1].icon}</div>
                                                <div className="absolute -top-1 -right-1 w-5 h-5 bg-green-500 rounded-full flex items-center justify-center text-[8px] text-white font-bold">{heroData.accessories[1].tag}</div>
                                            </div>
                                        </>
                                    )}
                                </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 pt-8 px-4">
                            {heroData.stats.map((stat) => (
                                <div key={stat.id} className="p-3 rounded-lg bg-gray-50 border border-gray-200 shadow-sm">
                    ...
                    <div className="hidden lg:grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
                            ...
                            <div>
                                ...
                                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-gray-900">
                                    ...
                                    <span className="relative">
                                        <span className="relative z-10 bg-gradient-to-r from-[#F8AE1B] via-yellow-400 to-[#F8AE1B] bg-clip-text text-transparent animate-shimmer">{heroData.subtitle}</span>
                                    </span>
                                </h1>
                            </div>
                            <p className="text-xl text-gray-700 leading-relaxed max-w-2xl bg-gray-50/80 p-6 rounded-xl border border-gray-200 backdrop-blur-sm shadow-sm">{heroData.description}</p>
                        ...
                        <div className="relative">
                            <div className="relative mx-auto w-full max-w-md">
                                <div className={`relative z-10 ${heroData.settings.animations ? 'animate-phone-float' : ''}`}>
                                    <div className="relative w-80 h-[580px] mx-auto bg-gradient-to-b from-gray-900 to-black rounded-[3rem] shadow-2xl border-[16px] border-gray-800 transform rotate-3 overflow-hidden">
                                        {heroData.settings.backgroundEffects && (
                                        ...
                                        {/* Dynamic IMAGE RENDER DESKTOP */}
                                        {heroData.featuredProduct.heroImage && (
                                            <img src={heroData.featuredProduct.heroImage} alt={heroData.featuredProduct.name} className="absolute inset-0 w-full h-full object-cover" />
                                        )}

                                        {/* SHARED DATA OVERLAY DESKTOP */}
                                        <div className="absolute inset-4 bg-gradient-to-b from-black/20 via-transparent to-black/60 rounded-[2rem] overflow-hidden">
                                            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#F8AE1B] via-yellow-400 to-[#F8AE1B] animate-shimmer"></div>
                                            <div className="relative py-8 px-4 h-full flex flex-col justify-between">
                                                <div>
                                                    ...
                                                    <div className="text-white text-3xl font-bold mb-2 drop-shadow-lg">{heroData.featuredProduct.name}</div>
                                                    <div className="text-4xl font-bold bg-gradient-to-r from-[#F8AE1B] to-yellow-400 bg-clip-text text-transparent drop-shadow-lg">{heroData.featuredProduct.model}</div>
                                                    <div className="bg-[rgba(0,0,0,0.7)] p-1 rounded-lg text-gray-100 text-sm mt-2 flex items-center gap-2 drop-shadow-lg"><FiZap className="text-[#F8AE1B]" />{heroData.featuredProduct.tagline}</div>
                                                </div>
                                        ...
                                        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-40 h-8 bg-black rounded-b-2xl border-x border-b border-gray-700 z-30"></div>
                                    </div>
                                    
                                    {heroData.settings.floatingElements && heroData.settings.animations && (
                                        <>
                                            <div className="absolute -top-6 -left-10 w-24 h-24 bg-gradient-to-br from-gray-800 to-black rounded-2xl border border-gray-700/50 flex items-center justify-center shadow-xl animate-float z-40" style={{ animationDelay: '0.5s' }}>
                                                <div className="text-3xl text-white">{heroData.accessories[0].icon}</div>
                                                <div className="absolute -bottom-2 -right-2 w-6 h-6 bg-[#F8AE1B] rounded-full flex items-center justify-center text-xs text-black font-bold">{heroData.accessories[0].tag}</div>
                                            </div>
                                            <div className="absolute -bottom-6 -right-10 w-28 h-28 bg-gradient-to-br from-gray-800 to-black rounded-2xl border border-gray-700/50 flex items-center justify-center shadow-xl animate-float z-40" style={{ animationDelay: '1s' }}>
                                                <div className="text-3xl text-white">{heroData.accessories[1].icon}</div>
                                                <div className="absolute -top-2 -right-2 w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center text-xs text-white font-bold">{heroData.accessories[1].tag}</div>
                                            </div>
                                        </>
                                    )}
                                </div>
                                {heroData.settings.animations && (
                                    <>
                                        <div className="absolute top-20 right-10 w-4 h-4 bg-[#F8AE1B] rounded-full animate-particle z-0"></div>
                                        <div className="absolute bottom-32 left-12 w-3 h-3 bg-blue-500 rounded-full animate-particle z-0" style={{ animationDelay: '0.7s' }}></div>
                                        <div className="absolute top-40 left-20 w-2 h-2 bg-yellow-400 rounded-full animate-particle z-0" style={{ animationDelay: '1.4s' }}></div>
                                    </>
                                )}
                            </div>
                        </div>
